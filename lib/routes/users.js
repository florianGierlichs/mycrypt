...
router.post('/coin/stock', async (request, response) => {
  const {
  ...
  try {
    ...
    const currentUserCoins = currentUser.coins;
    const filteredCoins = currentUserCoins.filter((item) => item.name !== coin);

    await User.findOneAndUpdate(
      { username: username },
      { coins: [...filteredCoins, { name: coin, stock: Number(stock) }] },
      {
        new: true,
        upsert: true,
      }
